package com.stafeewa.photocalorie.app.domain.usecase.recommendation
...
class GetRecommendationsUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        recentlyRecommendedProducts: List<String> = emptyList()
    ...
    private fun selectRecommendations(
        ...
        recentlyRecommendedProducts: List<String>
        ...
        val selectedPool = if (top10.isNotEmpty() && Random.nextDouble() < 0.2) top10.shuffled()
            .take(6) else scored.take(6)
        ...
        for ((product, _) in selectedPool) {
            ...
            selected += RecommendationItem(
                product,
                buildReason(product, remainingProtein, remainingFat, feedbackMap)
            )
    ...
    private fun scoreProduct(
        ...
        hour: Int,
        preferences: UserFoodPreferences?,
        feedbackMap: Map<String, List<RecommendationFeedback>>,
        recentlyRecommendedProducts: List<String>
    ): Double {
        val name = product.name.lowercase()
        val category = detectCategory(product).lowercase()
        if (preferences?.excludedProducts?.contains(name) == true || preferences?.excludedCategories?.contains(
                category
            ) == true
        ) return 0.0
        ...
        var score = sat(p, remainingProtein) * 0.35 + sat(f, remainingFat) * 0.25 + sat(
            c,
            remainingCarbs
        ) * 0.25 + sat(min(remainingCalories, cals), remainingCalories) * 0.15
        score *= mealTimeBoost(product.mealType, hour)
        if (recentlyRecommendedProducts.any { it.equals(product.name, true) }) score *= 0.6
        if (preferences?.preferredProducts?.contains(name) == true || preferences?.preferredCategories?.contains(
                category
            ) == true
        ) score *= 1.2
    ...
    private fun diversityBonus(product: Product, entries: List<FoodEntry>): Double {
        val usedNames = entries.map { it.name.lowercase() }
        val isNameRepeat = usedNames.any { it == product.name.lowercase() }
        val sameCategory =
            entries.count { detectCategoryByName(it.name) == detectCategory(product) }
        val keywordOverlap =
            product.keywords.count { keyword -> usedNames.any { it.contains(keyword.lowercase()) } }
    ...
    private fun buildReason(
        ...
        remainingProtein: Double,
        remainingFat: Double,
        feedback: Map<String, List<RecommendationFeedback>>
    ): String {
    ...
    private fun groupProductsByCategory(products: List<Product>): Map<String, List<Product>> =
        products.groupBy { detectCategory(it) }
